...
c
	real wx0(im,jm),wy0(im,jm),wrad(im,jm)
C
C
	common/vars/beta,dx,dy,g12,g23,f0,
     &         fh(jm),fv(jm),wx(im,jm),wy(im,jm)
c
	common/vars1/je,w0,wekman
c
c.......open plot file......
c
	call newdev('prob2.ps',8)
	call psinit(.true.)
c
	open(unit=15,file='ex3_prob2.input',status='old')
c
...
c
c  Set up a wind stress that produces a circular patch of
c  wind stress curl consistent with Rhines and Young (1982).
c  Try wx=alpha*rho0*f0*x*y.
c
c  Density of sea water (cgs).
c
       rho0=1.025
c
       wekman=100.*wekman
...
c
        if(ipert.eq.3)then
          iekman=rekman/dx
          jekman=rekman/dy
          print *,'iekman=',iekman,' ie=',ie
          print *,'jekman=',jekman,' je=',je
          do j=1,jm
            do i=1,im
             dfx=float(i-ie)*dx
             dfy=float(j-je)*dy
             wrad(i,j)=sqrt(dfx*dfx+dfy*dfy)/rekman
            enddo
          enddo
          do j=1,jm
            do i=1,im
            ...
            enddo
          enddo
          do j=je-jekman,je+jekman
            do i=ie-iekman,ie+iekman
              if(wrad(i,j).le.1.0) then
              wx0(i,j)=alpha*rho0*f0*float(i-ie)*float(j-je)*dx*dy
              endif
            enddo
          enddo
          do i=ie-iekman,ie+iekman
            iff=0
            do j=1,je
              if(wrad(i,j).le.1.0.and.iff.eq.0) then
                jslim=j
                iff=1
              endif
            enddo
...
c
	if(ipert.eq.2.or.ipert.eq.3)then
c
	wfac=1.0
