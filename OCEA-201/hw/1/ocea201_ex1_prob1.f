...
c
	...
	call psinit(.true.)
c
	open(unit=15,file='ocea201_ex1_prob1.input',status='old')
c
c
...
c
	if(ipert.eq.2)then
	  do j=1,jm
	   do i=1,im
	     wx(i,j)=0.5*sin(2.*4.*atan(1.0)*float(j-je)/float(2*jm))
c	     wx(i,j)=-0.1
	     wy(i,j)=0.
	   enddo
	  enddo
...
C      STUA(I)=DVT*(UKM(I,J) - U2(I,J,KM))
...
C      STVA(I)=DVT*(V1(I,J,KM) - V2(I,J,KM))
...
c
	do j=1,jm
	 do i=1,im
	   speed(i,j)=sqrt(u(i,j)*u(i,j)+v(i,j)*v(i,j))/100.
	   up(i,j)=0.0
	   vp(i,j)=0.0
	 enddo
	enddo
c
	do j=2,jm-1
	   do i=2,im-1
	   up(i,j)=0.5*(u(i,j)+u(i-1,j))/100.
	   vp(i,j)=0.5*(v(i,j)+v(i-1,j))/100.
	   enddo
	enddo
c
...
c	   spd=sqrt(up(i,j)*up(i,j)+vp(i,j)*vp(i,j))/100.
	   spd=sqrt(u(i,j)*u(i,j)+v(i,j)*v(i,j))/100.
