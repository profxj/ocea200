...
C*****CIRCLE
      ...
      radi=rad*conver
      scrc=' '
      write(scrc,'(f8.2,'' '',f8.2,'' '',f8.2,'' Circle'')') xci,yci,
     +radi
      if(fill) then
        cmdstr='newpath '//scrc(1:lenstr(scrc,132))//' fill'
      else
        cmdstr='newpath '//scrc(1:lenstr(scrc,132))//' stroke'
      endif
      call filler
      return
      end
...
C Curwid is set in routine setlw
   ...
   10 continue
      ...
      if(closer)call linet(xarr(1),yarr(1))
      cmdstr='S'
      call filler
...
C  Start next char at .5 char width away
      argdeg=arg*180./pi
      cmdstr=' '
      write(cmdstr,'(f5.1,'' Xyprset'')')argdeg
      call filler
...
c  This routine finds actual length of string by eliminating trailing blanks
      ...
      do i=ls,1,-1
        is=i
        if(string(i).ne.char(32)) goto 10
      enddo
      is=0
   10 lenstr=is
      return
...
C  Determine start and end points for overbar
      ...
      cmdstr=' '
      write(cmdstr, '(''/Xpos Strlen '',F5.1,'' cos mul Xpos add def'')'
     +) ang
 
      call filler
      cmdstr=' '
      write(cmdstr, '(''/Ypos Strlen '',F5.1,'' sin mul Ypos add def'')'
     +) ang
...
c  Strlenss to Strlensb if Strlensb is longer
      ...
      cmdstr=' '
      write(cmdstr,
     +'(''/Xpos Strlen Strlenss add '',F5.1,'' cos mul Xpos add def'')')
     +ang
      call filler
      cmdstr=' '
      write(cmdstr,
     +'(''/Ypos Strlen Strlenss add '',F5.1,'' sin mul Ypos add def'')')
     +ang
      call filler
...
C  ALLOW NAME OF OUTPUT PLOT FILE TO BE KNOWN BY DCL
c     CALL LIB$SET_SYMBOL('PSNAME',FILEOUT)
...
c  VMS
c      open(iunit,file=fileout,form='formatted', recl=132,
c     +status='unknown',
...
c     call date(dat)
     ...
     +'%%Library Creator: Kevin E. Kohler <kevin@ocean.nova.edu>'
      ...
      call filler
        ...
        cmdstr='/Circle {/Rad exch def /Yc exch def /Xc exch def'
      call filler
...
C  Compute next character position
      cmdstr=' '
      write(cmdstr,'(f5.1,'' Xyprset0'')')ang
      call filler
