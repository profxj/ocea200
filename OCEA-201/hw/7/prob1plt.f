...
c
         parameter (m0=41,n0=25,mn0=m0*n0,m20=mn0,mx0=m0+n0,k0=1,
...
c
	 real xarr(n0),yarr(n0)
c
	 real xlarr(2),ylarr(2)
c
	real spv(2),cfill(3),grylev(3),cfill1(3),grylev1(3)
c
        complex cfreq
c
	character ccrap*2,critz*10
c
...
c
        call newdev('prob1.ps',7)
	call psinit(.true.)
        call factor(fact)
c
        open(unit=10,file='ftnmplt.dat',form='unformatted',status='old')
c
	read(10)uprof
	read(10)gamma
c
...
c
         ...
         do j=1,n0
         do i=1,mm0
            if(nfld.eq.1)ap(i,j)=bstm(i,j,1)
            if(nfld.eq.2)ap(i,j)=amp*si(i,j,1,iplt)
            if(nfld.eq.3)ap(i,j)=bstm(i,j,1)+amp*si(i,j,1,iplt)
            if(nfld.eq.4)ap(i,j)=amp*vi(i,j,1,iplt)
            if(nfld.eq.5)ap(i,j)=bvrt(i,j,1)+amp*vi(i,j,1,iplt)
         enddo
         enddo
c
...
c
	...
	write(camup,'(1p,e10.3)')amuq
        write(cper,'(1p,e8.1)')eper
        write(cef,'(1p,e8.1)')efold
c
	print *,'iplt=',iplt
	print *,'amuq=',amuq
	print *,'amue=',amue
...
c
        if(nfld.eq.1)then
        call keksym(0.5*xlen,1.2*ylen,0.3,13HNormal mode #,0.,13,1)
	write(cnum,'(i1)')nplt
...
c
	...
	call keksymc(999.,999.,ch1,critz,0.,10,0)
	dyl=0.4
        call keksym(xlen+0.2,0.5*ylen,ch1,7HPeriod=,0.,7,0)
        call keksymc(999.,999.,ch1,cper,0.,10,0)
        call keksym(xlen+0.2,0.5*ylen-dyl,ch1,
     &     15He-folding time=,0.,15,0)
...
c
c         if(mod(iplt,5).ne.0)call plot(0.,ylen+0.5,-3)
         call plot(0.,-1.*(ylen+0.5),-3)
c
         if(nfld.eq.5)then
               call chopit(0.,0.)
               call plot(xstrt,ystrt,-3)
