...
c
c\Revision history:
c     12/16/92: Version ' 2.1'.
c
c\SCCS Information: @(#) 
c FILE: getv0.F   SID: 2.3   DATE OF SID: 11/21/95   RELEASE: 2
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
      ...
      integer    idist, iseed(4), iter, msglvl
      Real
     &           rnorm0
...
c     %----------------------%
c
      external   saxpy, scopy, sgemv, slarnv, svout, second
c
c     %--------------------%
...
c
c\SCCS Information: @(#) 
c FILE: laqrb.F   SID: 1.3   DATE OF SID: 11/21/95   RELEASE: 1
c
c\Remarks
c     1. None
c
...
c
      Real
     &           zero, one, dat1, dat2
      parameter (zero = 0.0, one = 1.0, dat1 = 0.75, dat2 = -0.4375)
c
c     %------------------------%
c     | Local Scalars & Arrays |
c     %------------------------%
...
c
c\SCCS Information: @(#) 
c FILE: naitr.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
...
c 
         if (msglvl .gt. 4) then
            call smout (logfil, j, j, h, ldh, ndigit, 
     &                  '_naitr: current upper Hessenberg matrix H')
         end if
         if (msglvl .gt. 5) then
           call smout (logfil, n, j, v, ldv, ndigit,
     &                  '_naitr: updated Arnoldi vectors')
            call svout (logfil, n, resid, ndigit, 
     &                  '_naitr: updated residual vector')
         end if
c
         if (msglvl .gt. 3) then
            call sgemv ('T', n, j, one, v, ldv, workd(ipj), 
     &                       1, zero, workd(irj), 1)
            if (rnorm .ge. unfl) then
                temp1 = one/rnorm
                call sscal(j, temp1, workd(irj), 1)
            else if (rnorm .gt. zero) then
                call slascl ('General', i, i, rnorm, one, j, 1, 
     &                      workd(irj), j, infol)
            else 
                call sscal(j, zero, workd(irj), 1) 
            end if
            call svout (logfil, j, workd(irj), ndigit, 
     &               '_saitr: V^T * B * (resid/B-norm(resid))')
         end if
c
c 
c        %------------------------------------%
...
c
c\Routines called:
c     slarf   LAPACK Householder reflection application routine.
...
c
c\SCCS Information: @(#) 
c FILE: napps.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
...
c
         if (msglvl .gt. 3) then
            call smout (logfil, kplusp, kplusp, h, ldh, ndigit,
...
c
c              %------------------------------------------------------%
c              | Construct Householder reflection G to zero out u(1). |
...
c
c              %--------------------------------------------%
c              | Apply relector to the left of H;  H <- G*H |
c              %--------------------------------------------%
c
...
c
c              %----------------------------------------------%
c              | Apply relector to the right of H;  H <- H*G' |
c              %----------------------------------------------%
c
               ir = min ( i+3, iend )
...
c
      do 120 j=1,kplusp-1
         if ( h(j+1,j) .lt. zero ) then
              ...
              call sscal( kplusp, -one, q(1,j+1), 1 )
         end if
 120  continue
c
      do 130 i = 1, kev-1
...
c
      if (msglvl .gt. 4) then
         call smout (logfil, kplusp, kplusp, q, ldq, ndigit,
...
c
     ...
     &        '_napps: updated residual for next iteration')
      ...
      if (msglvl .gt. 4) then
         call smout (logfil, n, kev+1, v, ldv, ndigit,
...
c 
c\Revision history:
c     xx/xx/92: Version '1.1'.  (Phuong Vu)
c     xx/xx/94: Version '1.1'.  (R.B. Lehoucq)
c     xx/xx/95: Version '1.1'.  (R.B. Lehoucq)
c
c\SCCS Information: @(#) 
c FILE: naup2.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     1. None
c
...
c
     ...
     &           '_naup2: first NEV Arnoldi steps at iteration no.')
         end if
      end if
c 
c     %-----------------------------------%
...
c        | acceptable if:                                             |
c        |                                                            |
c        |        error_bounds(i) .le. tol*magnitude_of_ritz(i).      |
c        |                                                            |
c        | This is not as elaborate as the test used in the symmetric |
c        | version, i.e. no refined error bounds, gap test, etc..     |
c        %------------------------------------------------------------%
c
...
c
            nswap = min ( nconv, kplusp-nconv )
            if ( nconv .gt. 0 ) then
               call sswap (nswap, ritzr(kplusp-nswap+1), 1, ritzr, 1) 
               call sswap (nswap, ritzi(kplusp-nswap+1), 1, ritzi, 1) 
               call sswap (nswap, bounds(kplusp-nswap+1), 1, bounds, 1)
            end if
            h(3,1) = rnorm
            if (nconv .gt. np0) 
     &          call ssortc (which, .true., nconv, ritzr, ritzi, 
     &                bounds) 
c
c           %------------------------------------%
...
c
      if (msglvl .gt. 0) then
         call ivout (logfil, 1, nconv, ndigit, 
     &        '_naup2: Number of "converged" Ritz values')
         call svout (logfil, nconv, ritzr, ndigit,
     &        '_naup2: Real part of the "converged" Ritz values')
         call svout (logfil, nconv, ritzi, ndigit,
     &        '_naup2: Imaginary part of the "Converged" Ritz values')
         call svout (logfil, nconv, bounds, ndigit,
     &        '_naup2: Associated Ritz estimates')
         if (msglvl .gt. 2) then
            call svout (logfil, kplusp-np, ritzr(np+1), ndigit,
     &           '_naup2: Real part of the remaining Ritz values')
            call svout (logfil, kplusp-np, ritzi(np+1), ndigit,
     &           '_naup2: Imaginary part of the Remaining Ritz values')
            call svout (logfil, kplusp-np, bounds(np+1), ndigit,
     &           '_naup2: Associated Ritz estimates')
         end if 
      end if
c
...
c
c          IPARAM(2) = LEVEC
c          LEVEC .LE. 0: Only the eigenvalues are to be computed.
c          LEVEC .GT. 0: Both eigenvalues and a basis for an invariant
c                        subspace are to be computed. (See remark 2 below)
c
c          IPARAM(3) = MXITER
...
c                     sseupd if RVEC = .TRUE. See Remark 2 below.
c          Note: IPNTR(9:13) is only referenced by sneupd. See Remark 2 below.
...
c
c\SCCS Information: @(#) 
c FILE: naupd.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c
c\EndLib
...
c     %----------------------%
c
      external   snaup2, sscal, svout, ivout, second, sstatn
c
c     %--------------------%
c     | External Functions |
c     %--------------------%
...
c        %-----------------------------%
c
         call sscal ( 3*ncv**2 + 6*ncv, zero, workl, 1 )
c 
c        %-------------------------------------------------------------%
...
c
 ...
 1100    format (
     ...
     &      5x, 'Total time in snaup2 routine               = ', f12.6,/
...
c
c\SCCS Information: @(#) 
c FILE: nconv.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     1. xxxx
c
...
c
c\SCCS Information: @(#) 
c FILE: neigh.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     None
c
...
c
c  Two of these parameters (V, INFO) are also output parameters:
c
...
c 
c\SCCS Information: @(#) 
c FILE: neupd.F   SID: 2.3   DATE OF SID: 11/21/95   RELEASE: 2 
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
     ...
     &           irr, iri, ibd
      logical    reord
      Real
     &           conds, rnorm, sep, temp, thres, vl(1,1), temp1
c
c     %----------------------%
c     | External Subroutines |
c     %----------------------%
...
c
     ...
     &      '_neupd: REAL part of the Ritz values from _neigh.')
         call svout(logfil, ncv, workl(iri),ndigit,
     &      '_neupd: IMAG part of the Ritz values from _neigh.')
         call svout(logfil, ncv, workl(ibd),ndigit,
     &        '_neupd: RITZ ESTIMATES from _neigh.')
      end if
c
c     %------------------------------------%
...
c     
      if (msglvl .gt. 0) then
         call ivout (logfil, 1, nconv, ndigit, 
     &        '_neupd: The number of "converged" Ritz values')
         call svout (logfil, 1, rnorm, ndigit, 
     &        '_neupd: Corresponding B-norm of the residual')
         call svout (logfil, ncv, workl(ritzr), ndigit,
     &        '_neupd: Real part of the Ritz values from _naupd')
         call svout (logfil, ncv, workl(ritzi), ndigit,
     &        '_neupd: Imaginary part of the Ritz values from _naupd')
         call svout (logfil, ncv, workl(bounds), ndigit, 
     &        '_neupd: Ritz estimates of the NCV Ritz values')
         if (msglvl .gt. 1) then
            call smout (logfil, ncv, ncv, workl(ih), ldh, ndigit,
     &           '_neupd: The upper Hessenberg matrix H')
            if (msglvl .gt. 4) then
               call ivout(logfil, 13, ipntr, ndigit, 
     &              '_neupd: Integer array of pointers')
            end if
         end if
      end if 
c
...
c
     ...
     &            .ge. thres) then
                  temp1 = slapy2( workl(irr+j), workl(iri+j) )
...
c
c           %---------------------------------%
c           | More than NCONV Ritz values has |
c           | "converged", and they all       |
c           | satisfy the WHICH selection     |
c           | criterion.                      | 
c           %---------------------------------%
c
c           iparam(6) = ktrord
...
c     
     ...
     &                 ndigit, '_neupd: The associated Schur matrix ')
               end if
            end if
         end if 
c
...
c
         if (msglvl .gt. 4) then
            call smout (logfil, ncv, nconv, workl(invsub), ldq, ndigit,
     &                 '_neupd: The factorized invariant subspace')
         end if
c     
c        %---------------------------------------------------------%
...
c     
     ...
     &                 workl(invsub+(j-1)*ldq), 1 )
                  workl(ihbds+j-1) = abs( workl(invsub+ncv-1+(j-1)*ldq) 
     &                                  )
               else
c     
c                 %-------------------------------------------%
...
c     
     ...
     &                      workl(invsub+j*ldq), 1 )
                     workl(ihbds+j-1) = slapy2( 
     &                                  workl(invsub+ncv-1+(j-1)*ldq),
     &                                  workl(invsub+ncv-1+j*ldq) )
...
c
         if (rvec) then
            ...
            call scopy (ncv, workl(bounds), 1, workl(ihbds), 1)
         end if
c     
      else 
c     
...
c
            do 50 k=1, ncv
               temp = slapy2( workl(iheigr+k-1), workl(iheigi+k-1) )
...
c     %-------------------------------------------------%
c
      if (howmny .eq. 'A' .and. type .eq. 'SHIFTI') then
c
c        %------------------------------------------------%
...
c
c\SCCS Information: @(#) 
c FILE: ngets.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     1. xxxx
c
...
c
c\SCCS Information: @(#) 
c FILE: saitr.F   SID: 2.3   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c  The algorithm implemented is:
c  
...
c 
         if (msglvl .gt. 2) then
            call svout (logfil, 1, rnorm, ndigit, 
     &                  '_saitr: iterative refinement ; rnorm is')
            call svout (logfil, 1, rnorm1, ndigit, 
     &                  '_saitr: iterative refinement ; rnorm1 is')
         end if
c
c        %-----------------------------------------%
c        | Determine if we need to perform another |
...
c 
         if (msglvl .gt. 3) then
            call svout (logfil, j, h(1,2), ndigit, 
     &                  '_saitr: main diagonal of matrix H')
            if (j .gt. 1) then
            call svout (logfil, j-1, h(2,1), ndigit, 
     &                  '_saitr: sub diagonal of matrix H')
            end if
         end if
c
         if (msglvl .gt. 3) then
            call svout (logfil, n, resid, ndigit, 
     &                  '_saitr: updated residual vector')
         end if
c
         if (msglvl .gt. 3) then
            call smout (logfil, n, j, v, ldv, ndigit,
     &                  '_saitr: updated Arnoldi vectors')
         end if
c
         if (msglvl .gt. 2 .and. rnorm .gt. zero) then
            call sgemv ('T', n, j, one, v, ldv, workd(ipj), 
     &                       1, zero, workd(irj), 1)
            if (rnorm .ge. safmin) then
                temp1 = one/rnorm
...
c
         if (h(j,1) .lt. zero) then
            if (msglvl .gt. 0) then
               call svout (logfil, 1, h(j,1), ndigit, 
     &                  '_saitr: current off-diagonal is negative')
            end if
            h(j,1) = -h(j,1)
            call sscal(n, -one, v(1,j), 1)
         end if
c 
c        %------------------------------------%
c        | STEP 6: Update  j = j+1;  Continue |
...
c
     ...
     &         '_saitr: sub diagonal of matrix H of step K+NP.')
               ...
               call sgemv ('T', n, k+np, one, v, ldv, workd(ipj), 
     &                1, zero, workd(irj), 1)
               if (rnorm .ge. safmin) then
                   temp1 = one/rnorm
                   call sscal(j, temp1, workd(irj), 1)
               else if (rnorm .gt. zero) then
                   call slascl ('General', i, i, rnorm, one, k+np, 1, 
     &                           workd(irj), k+np, infol)
               end if
               call svout (logfil, k+np, workd(irj), ndigit, 
     &                 '_saitr: V^T * B * (resid/B-norm(resid))')
            end if
c
            go to 9000
...
c
c\SCCS Information: @(#) 
c FILE: sapps.F   SID: 2.4   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
...
c
      if (msglvl .gt. 1) then
         call svout (logfil, kplusp, h(1,2), ndigit, 
     &     '_sapps: main diagonal of trid matrix H before any shifts.')
         if (kplusp .gt. 1) then
         call svout (logfil, kplusp-1, h(2,1), ndigit, 
     &        '_sapps: subdiagonal of trid matrix H before any shifts')
         end if
      end if
c
      do 90 jj = 1, np
...
c 
c           %------------------------------------------------------%
c           | Contruct the plane rotation G(istart,istart+1,theta) |
...
c 
c               %--------------------------------------------%
c               | Contruct the plane rotation G(i,i+1,theta) |
...
c
         if (h(iend,1) .lt. zero) then
             h(iend,1) = -h(iend,1)
             call sscal(kplusp, -one, q(1,kplusp), 1)
         end if
c
c        %--------------------------------------------------------%
c        | Apply the same shift to the next block if there is any |
...
c
         if (msglvl .gt. 2) then
            call ivout (logfil, 1, jj, ndigit, 
     &          '_sapps: shift number applied.')
            call svout (logfil, 1, shift(jj), ndigit, 
     &          '_sapps: corresponding shift applied.')
            call svout (logfil, kplusp, h(1,2), ndigit, 
     &          '_sapps: main diagonal of H after this shift.')
            if (kplusp .gt. 1) then
            call svout (logfil, kplusp-1, h(2,1), ndigit, 
     &          '_sapps: subdiagonal of H after this shift.')
            end if
         end if
c
c        %-----------------------------------------------------%
...
c     %------------------------------------------------------%
c
         do 110 i = itop, kplusp-1
            if (h(i+1,1) .gt. zero) go to 120
            itop  = itop + 1
 110     continue
 120     continue
c
...
c
     ...
     &      '_sapps: updated sub diagonal of H for next iteration')
      ...
      if (msglvl .gt. 3) then
         call svout (logfil, n, resid, ndigit, 
     &      '_sapps: updated residual for next iteration')
      end if
c 
c      call second (t1)
      tsapps = tsapps + (t1 - t0)
...
c     xx/xx/95: Version ' 2.1'.  (R.B. Lehoucq)
c
c\SCCS Information: @(#) 
c FILE: saup2.F   SID: 2.4   DATE OF SID: 11/21/95   RELEASE: 2
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
      logical    cnorm, getv0, initv, update, ushift
      integer    i, ierr, iter, j, kplusp, msglvl, nconv, nevbef, nev0, 
     &           np0, nptemp, nswap, kp(2) 
      Real
     &           rnorm
...
c
     ...
     &           ssapps, ssortr, svout, ivout, second
c
c     %--------------------%
c     | External Functions |
c     %--------------------%
...
c
         ...
         if (msglvl .gt. 0) then
            call ivout (logfil, 1, iter, ndigit, 
     &           '_saup2: first NEV Lanczos steps at iteration no.')
         end if
      end if
c 
c     %------------------------------------------------------------%
...
c 
         if (msglvl .gt. 2) then
            call svout (logfil, kplusp, h(1,2), ndigit,
     &           '_saup2: main diagonal of current H matrix')
            call svout (logfil, kplusp-1, h(2,1), ndigit, 
     &           '_saup2: subdiagonal of current H matrix')
         end if
c
c        %--------------------------------------------------------%
c        | Compute the eigenvalues and corresponding error bounds |
...
c 
     ...
     &           '_saup2: Ritz values of the tridiagonal H matrix')
            call svout (logfil, kplusp, bounds, ndigit, 
     &        '_saup2: Ritz estimates of the NCV Ritz values')
         end if
c 
c        %----------------------%
...
c
         if (msglvl .gt. 0) then
            call ivout (logfil, 1, nconv, ndigit, 
     &           '_saup2: Number of "converged" Ritz values')
         end if
c
c        %---------------------------------------------------------%
c        | Count the number of unwanted Ritz values that have zero |
...
c     
            if (nconv .gt. 0) then
               nswap = min ( nconv, kplusp-nconv )
               call sswap (nswap, ritz(kplusp-nswap+1), 1, ritz, 1) 
               call sswap (nswap, bounds(kplusp-nswap+1), 1, bounds, 1)
               call ssortr ('LA', .true., nconv, ritz, bounds) 
            end if
            h(1,1) = rnorm
c
c           %------------------------------------%
...
c
     ...
     &                  '_saup2: shifts selected')
            if (msglvl .gt. 2) then
               call svout (logfil, np, ritz, ndigit,
     &                  '_saup2: "un-wanted" Ritz values')
               call svout (logfil, np, bounds, ndigit,
     &                  '_saup2: corresponding Ritz estimates')
             end if
         end if
...
c 
         if (msglvl .gt. 4) then
            call scopy (nev, h(1,2), 1, workl, 1)
            if (nev .gt. 1) 
     &         call scopy (nev-1, h(2,1), 1, workl(nev+1), 1)
            call sstqrb (nev, workl, workl(nev+1), workl(2*nev+1),
     &                   workd(n+1), ierr)
            if (nev .gt. 1) then
               do 140 i=1, nev
                  workl(2*nev+i) = abs(workl(2*nev+i))
  140          continue
               call sscal (nev, rnorm, workl(2*nev+1), 1)
            else 
               workl(2*nev+1) = rnorm
            end if
            if (ierr .ne. 0) then 
               call ivout (logfil, 1, ierr, ndigit, 
     &         '_saup2: informational error in call sstqrb')
            else
               call svout (logfil, nev, workl, ndigit,
     &           '_saup2: Ritz values of the order NEV H matrix')
               call svout (logfil, nev, workl(2*nev+1), ndigit, 
     &         '_saup2: corresponding error bounds of the Ritz values')
            end if
         end if
c
      go to 1000
...
c
      if (msglvl .gt. 0) then
         call svout (logfil, 1, rnorm, ndigit, 
     &        '_saup2:  Residual norm before exit')
         call svout (logfil, np, ritz, ndigit, 
     &        '_saup2:  Ritz values before exit')
         call svout (logfil, np, bounds, ndigit, 
     &        '_saup2:  Associated Ritz estimates before exit')
         call ivout (logfil, 1, nconv, ndigit, 
     &        '_saup2:  Number of converged Ritz values')
      end if
c
...
c
c\SCCS Information: @(#) 
c FILE: saupd.F   SID: 2.4   DATE OF SID: 11/21/95   RELEASE: 2 
c
c\Remarks
c     1. None
c
...
c     %----------------------%
c
      external   ssaup2, sscal, svout, ivout, second, sstats
c
c     %--------------------%
c     | External Functions |
c     %--------------------%
...
c        %-----------------------------%
c
         call sscal ( ncv**2 + 8*ncv, zero, workl, 1 )
c 
c        %-------------------------------------------------------%
c        | Pointer into WORKL for address of H, RITZ, BOUNDS, Q  |
c        | etc... and the remaining workspace.                   |
...
c
 ...
 1100    format (
     ...
     &      5x, 'Total time in user B*x operation           = ', f12.6,/
     &      5x, 'Total time in Arnoldi update routine       = ', f12.6,/
     &      5x, 'Total time in ssaup2 routine               = ', f12.6,/
     &      5x, 'Total time in basic Arnoldi iteration loop = ', f12.6,/
     &      5x, 'Total time in reorthogonalization phase    = ', f12.6,/
     &      5x, 'Total time in (re)start vector generation  = ', f12.6,/
...
c
c\SCCS Information: @(#) 
c FILE: sconv.F   SID: 2.3   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     1. Starting with version 2.3, this routine no longer uses the
...
c           | when: bounds(i) .le. TOL*abs( ritz(i) )       |
c           %-----------------------------------------------%
c
         if ( bounds(i) .le. tol*abs(ritz(i)) ) then
            nconv = nconv + 1
         end if
c
   10 continue
...
c
c\SCCS Information: @(#) 
c FILE: seigt.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     None
c
...
c
c     %--------------------------------------------%
c     | Find the last zero off-diagonal element to |
c     | determine the size of the last unreduced   |
c     | block starting at column/row number k.     |
c     %--------------------------------------------%
c    
       do 10 i = n, 2, -1
          if (h(i,1) .eq. zero) go to 20
   10  continue
       i = 1
   20  continue
c
c      %------------------------------------------------%
c      | Compute the order of the last unreduced block. |
c      %------------------------------------------------%
c    
       kk = n - i + 1
c
c      %-----------------------------------------------%
c      | Call the modified LAPACK routine to determine |
c      | the eigenvalues and last component of the     |
c      | corresponding eigenvectors of the last        |
c      | unreduced block of dimension kk.              |
c      %-----------------------------------------------%
c    
       call scopy  (kk, h(i,2), 1, eig(i), 1)
       if (kk.gt.1)
     &     call scopy  (kk-1, h(i+1,1), 1, workl(i), 1)
       call sstqrb (kk, eig(i), workl(i), bounds(i),
     &              workl(n+1), ierr)
       if (ierr .ne. 0) go to 9000
c
c      %-----------------------------------------------%
c      | Call the modified LAPACK routine to determine |
c      | the eigenvalues of H(1:n-kk,1:n-kk). The      |
c      | corresponding eigenvectors of this block have | 
c      | last component equal to zero.                 |
c      %-----------------------------------------------%
c
       if (n-kk.gt.0) then
           call scopy  (n-kk, h(1,2), 1, eig, 1)
           if (n-kk.gt.1)
     &         call scopy  (n-kk-1, h(2,1), 1, workl, 1)
           call sstqrb (n-kk, eig, workl, bounds,
     &                  workl(n+1), ierr)
           if (ierr .ne. 0) go to 9000
c
c          %---------------------------------------------%
c          | Set the corresponding bounds equal to zero. |
c          %---------------------------------------------%
c
           call sscal(n-kk, zero, bounds, 1)
       end if
c
      if (msglvl .gt. 1) then
         call svout (logfil, n, bounds, ndigit,
...
c
c\SCCS Information: @(#) 
c FILE: sesrt.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
c\SCCS Information: @(#) 
c FILE: seupd.F   SID: 2.4   DATE OF SID: 11/21/95   RELEASE: 2
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
      character  type*6
      integer    bounds, ierr, ih, ihb, ihd, imin, iq, iw, j, k, 
     &           ldh, ldq, mode, msglvl, nconv, ncnvd2, next, ritz
      Real
     &           bnorm2, rnorm, temp
c
c     %----------------------%
...
c     %----------------------%
c
      external   scopy, sger, sgeqr2, slacpy, sorm2r, sscal, ssesrt, 
     &           ssteqr, sswap, svout
c
c     %--------------------%
c     | External Functions |
...
c
c        %-----------------------------------------------------------%
c        | Call LAPACK routine ssteqr to compute the eigenvalues and |
...
c
     ...
     &           '_seupd: main diagonal of the matrix H')
            if (ncv .gt. 1) then
                call svout (logfil, ncv-1, workl(ihb), ndigit,
     &              '_seupd: subdiagonal of the matrix H')
            end if
         end if
c
...
c
c        %--------------------------------------------------%
c        | Sort Ritz values and vectors according to WHICH  |
c        | The NCONV converged Ritz values and vectors are  |
c        | placed in the first NCONV locations. For example |
c        | if WHICH = 'LM', sort using an 'SM' ordering     |
c        | that leaves the wanted part in the first NCONV   |
c        | spots. No reason to sort if WHICH = 'SA' since   |
c        | ssteqr already computes a 'LA' ordering.         |
c        %--------------------------------------------------%
c
         if (which .eq. 'BE') then 
c
...
c           | to the low end in appropriate locations.            |
c           | BE CAREFUL, Only swap minimum of                    |
c           | ( ceiling( nconv/2 ), ncv-nconv ) values to avoid   |
c           | swapping overlapping locations.                     |
c           %-----------------------------------------------------%
c
            call ssesrt( 'LA', .true. , ncv, workl(ihd), ncv, workl(iq), 
     &                   ldq )
            ncnvd2 = nconv - nconv / 2 
            imin = min ( ncnvd2 , ncv-nconv )
            call sswap (imin, workl(ihd+nconv/2), 1, 
     &           workl(ihd+ncv-imin), 1)
            do 10 j=0, imin-1
               call sswap ( ncv, workl(iq+(nconv/2+j)*ldq), 1, 
     &              workl(iq+(ncv-imin+j)*ldq), 1 )
 10         continue
c
         else if (which .eq. 'LM') then 
c
...
c           |               LM, SM case.                       |
c           | Sort the eigenvalues of H into the desired order |
c           | The eigenvalues are sorted so that the wanted    |
c           | part are always in the first NCONV locations.    |
c           %--------------------------------------------------%
c
            call ssesrt( 'SM', .true. , ncv, workl(ihd), ncv, 
     &                   workl(iq), ldq )
         else if (which .eq. 'SM') then 
            call ssesrt( 'LM', .true. , ncv, workl(ihd), ncv, 
     &                   workl(iq), ldq )
         else if (which .eq. 'LA') then
c
c           %--------------------------------------------------%
c           |               LA case.                           |
c           | No need to sort. Only need to reflect ordering   |
c           | about "center", i.e. swap first and last, second |
c           | and next to the last, etc..                      |
c           %--------------------------------------------------%
c
            do 20 j=1, ncv/2
               call sswap (ncv, workl(iq+(j-1)*ldq), 1, 
     &                     workl(iq+(ncv-j)*ldq), 1)
               temp             = workl(ihd+j-1)
               workl(ihd+j-1)   = workl(ihd+ncv-j)
               workl(ihd+ncv-j) = temp
  20        continue    
         end if
c
c        %----------------------------------------%
...
c
         if (msglvl .gt. 1) then
            call scopy(ncv, workl(iq+ncv-1), ldq, workl(iw), 1)
            call svout (logfil, ncv, workl(iw), ndigit,
     &    '_seupd: Reordered last row of the eigenvector matrix for H')
            call svout (logfil, ncv, workl(ihd), ndigit,
     &    '_seupd: Reordered NCV Ritz values of the final H matrix')
         end if
c
      else
c
...
c        %-----------------------------------------------------%
c
         call sscal (ncv-1, zero, workl(ihb), 1)
         workl(ihb+ncv-1) = one
...
c
c\SCCS Information: @(#) 
c FILE: sgets.F   SID: 2.3   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c
c\EndLib
...
c
c\SCCS Information: @(#) 
c FILE: sortc.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
c\SCCS Information: @(#) 
c FILE: sortr.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
c\EndLib
c
c-----------------------------------------------------------------------
...
c
c\SCCS Information: @(#) 
c FILE: statn.F   SID: 2.2   DATE OF SID: 11/21/95   RELEASE: 2
c
      subroutine sstatn
c
c     %--------------------------------%
...
c
c\SCCS Information: @(#) 
c FILE: stats.F   SID: 1.2   DATE OF SID: 11/21/95   RELEASE: 1
c     %---------------------------------------------%
c     | Initialize statistic and timing information |
...
c
c\SCCS Information: @(#) 
c FILE: stqrb.F   SID: 2.3   DATE OF SID: 11/21/95   RELEASE: 2
c
c\Remarks
c     1. Starting with version 2.3, this routine is a modified version
...
c     .. external subroutines ..
     ...
     &                   slasrt, sswap, xerbla, 
c
c    *** New starting with version 2.3 ***
c
     &                   sscal
c     ..
c     .. intrinsic functions ..
