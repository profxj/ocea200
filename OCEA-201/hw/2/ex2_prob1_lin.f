...
c
	...
	call psinit(.true.)
c
	open(unit=15,file='ex2_prob1_lin.input',status='old')
c
...
c
 ...
 21     CONTINUE
       DO 128 J=2,JMAXM
      DO 28 I=2,IMAXM
c      H1AX=HH1-(H1(I,J,K)+H1(I+1,J,K))/2.
c      H2AX=HH2+(H1(I,J,K)+H1(I+1,J,K))/2.-(H2(I,J,K)+H2(I+1,J,K))/2.
c      H1AY=HH1-(H1(I,J,K)+H1(I,J+1,K))/2.
c      H2AY=HH2+(H1(I,J,K)+H1(I,J+1,K))/2.-(H2(I,J,K)+H2(I,J+1,K))/2.
      H1AX=HH1
      H2AX=HH2
      H1AY=HH1
      H2AY=HH2
...
CDIR$ IVDEP
       DO 30 I=2,IMAXM
c      DEX1(I)=HH1-(HK(I,J)+HK(I+1,J))/2.
c      DEX2(I)=HH2-(HK2(I,J)+HK2(I+1,J)-HK(I,J)-HK(I+1,J))/2.
c
c.....linear code.....
c
      DEX1(I)=HH1
...
c     5  -(((UK(I+1,J)+UK(I,J))*(US1(I+1,J)+US1(I,J))-
c     6  (UK(I-1,J)+UK(I,J))*(US1(I-1,J)+US1(I,J)))/DX
c     7  +((VK(I+1,J)+VK(I,J))*(US1(I,J+1)+US1(I,J))-
c     8   (VK(I+1,J-1)+VK(I,J-1))*(US1(I,J)+US1(I,J-1)))/DY)/4.
     9         )
...
C    5  -(((UK2(I+1,J)+UK2(I,J))*(US2(I+1,J)+US2(I,J))-
C    6  (UK2(I-1,J)+UK2(I,J))*(US2(I-1,J)+US2(I,J)))/DX
C    7  +((VK2(I+1,J)+VK2(I,J))*(US2(I,J+1)+US2(I,J))-
C    8  (VK2(I+1,J-1)+VK2(I,J-1))*(US2(I,J-1)+US2(I,J)))/DY)/4.
     9    + STUA(I) - STUB(I)
     9         )
C     U1(I,J,KP)=U1(I,J,KM)+TDT*U1CH*IUM(I,J)
C     U2(I,J,KP)=U2(I,J,KM)+TDT*U2CH*IUM(I,J)
30    CONTINUE
       DO 31 J=2,JMAXM
CDIR$ IVDEP
       DO 31 I=2,IMAXM
c      DEY1(I)=HH1-(HK(I,J)+HK(I,J+1))/2.
c      DEY2(I)=HH2-(HK2(I,J)+HK2(I,J+1)-HK(I,J)-HK(I,J+1))/2.
c
c......Linear code.....
c
      DEY1(I)=HH1
...
c     5  -(((UK(I,J)+UK(I,J+1))*(VS1(I,J)+VS1(I+1,J))-
c     6   (UK(I-1,J)+UK(I-1,J+1))*(VS1(I-1,J)+VS1(I,J)))/DX
c     7  +((VK(I,J+1)+VK(I,J))*(VS1(I,J+1)+VS1(I,J))-
c     8  (VK(I,J-1)+VK(I,J))*(VS1(I,J-1)+VS1(I,J)))/DY)/4.
     9         )
...
C    5  -(((UK2(I,J)+UK2(I,J+1))*(VS2(I+1,J)+VS2(I,J))-
C    6  (UK2(I-1,J)+UK2(I-1,J+1))*(VS2(I-1,J)+VS2(I,J)))/DX
C    7  +((VK2(I,J+1)+VK2(I,J))*(VS2(I,J+1)+VS2(I,J))-
C    8  (VK2(I,J-1)+VK2(I,J))*(VS2(I,J-1)+VS2(I,J)))/DY)/4.
     9     + STVA(I) - STVB(I)
     9         )
C     V1(I,J,KP)=V1(I,J,KM)+TDT*V1CH*IVM(I,J)
C     V2(I,J,KP)=V2(I,J,KM)+TDT*V2CH*IVM(I,J)
31     CONTINUE
C
       DO 32 J=2,JMAXM
       DO 32 I=2,IMAXM
...
c.....Linear code.....
c
       h1ch=HH1*((US1(I,J)-US1(I-1,J))/DX+
     1 (VS1(I,J)-VS1(I,J-1))/DY) - DD*HKM(I,J)
c
c       h1ch=((UK(I,J)-UK(I-1,J))/DX+
c     1 (VK(I,J)-VK(I,J-1))/DY) - DD*HKM(I,J)
...
c
     ...
     1      h1ch+HH2*((US2(I,J)-US2(I-1,J))/DX+
     1 (VS2(I,J)-VS2(I,J-1))/DY) + DD*HKM(I,J)
c
c       HKP2(I,J)=HKM2(I,J)+TDT*IHM(I,J)*(
c     1      h1ch+((UK2(I,J)-UK2(I-1,J))/DX+
c     1 (VK2(I,J)-VK2(I,J-1))/DY) + DD*HKM(I,J)
...
c     5   +(HKM2(I,J-1)-HKM2(I,J))*IHM(I,J-1)*IHM(I,J))*RDY2)
     6          )
C     H1(I,J,KP)=H1(I,J,KM)+TDT*H1CH*IHM(I,J)
C     H2(I,J,KP)=H2(I,J,KM)+TDT*H2CH*IHM(I,J)
  32    CONTINUE
C
...
c
	real q1(im,jm),q2(im,jm),qgeos(im,jm),psib(im,jm)
c
...
c
	do j=1,jm-1
     ...
     &           -0.25*(h2(i,j)+h2(i+1,j)+h2(i,j+1)+h2(i+1,j+1))
	    ...
	    sver1(i,j)=hfl1*vfl1/pscal
	    sver2(i,j)=hfl2*vfl2/pscal
	...
	do j=1,jm-1
	   do i=im,1,-1
	      ...
	      do ii=im,i+1,-1
	         ...
	         sum2(i,j)=sum2(i,j)+sver2(ii,j)*dxm
...
c
	call keksym(-0.5,1.05*ylen,ch3,1HA,0.,1,0)
c
...
c
        call border(xlen,ylen,0000,1111,1,1,2,1)
c
	call keksym(-0.5,1.05*ylen,ch3,1HB,0.,1,0)
c
	call keksym(0.5*xlen,1.05*ylen,ch1,1Hh,0.,1,1)
...
c
	...
	enddo
c
c.......set the NW corner vectors to zero for layer 1....
c
c	do j=jm-4,jm
c	do i=1,4
c	  up(i,j)=0.0
...
c
	call keksym(-0.5,1.05*ylen,ch3,1HC,0.,1,0)
c
...
c
	...
	call subber(1H2,1,ch1,0.)
c
	call keksym(-0.5,1.05*ylen,ch3,1HD,0.,1,0)
c
	bigwn=-1.e20
	do j=1,jm
	  do i=1,im
...
c
        call border(xlen,ylen,0000,1111,1,1,2,1)
c
	call keksym(-0.5,1.05*ylen,ch3,1HE,0.,1,0)
c
...
c
        call border(xlen,ylen,0000,1111,1,1,2,1)
c
	call keksym(-0.5,1.05*ylen,ch3,1HF,0.,1,0)
c
c
c	call keksym(0.5*xlen,1.05*ylen+0.3,0.7*ch1,
c     &         28HZonally Integrated Northward,0.,28,1)
...
c
        call border(xlen,ylen,0000,1111,1,1,2,1)
c
	call keksym(-0.5,1.05*ylen,ch3,1HG,0.,1,0)
c
c
c	call keksym(0.5*xlen,1.05*ylen+0.3,0.7*ch1,
c     &         28HZonally Integrated Northward,0.,28,1)
...
c
        call border(xlen,ylen,0000,1111,1,1,2,1)
c
	call keksym(-0.5,1.05*ylen,ch3,1HH,0.,1,0)
c
c
c	call keksym(0.5*xlen,1.05*ylen+0.3,0.7*ch1,
...
c
	call grksym(0.5*xlen,1.05*ylen,ch1,47,0.,1,0)
